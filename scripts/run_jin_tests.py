#!/usr/bin/env python3
"""
Run Jin et al. parametric vs contextual knowledge tests properly.

Parametric (facts stored in weights):
- Cities, Sports, Art, Celebrity, Technology

Contextual (info from input context):
- IMDB (sentiment), AQUA (math reasoning), Passkey (retrieval)
"""
...
def load_qa_json(path: str, n: int = 100) -> List[Dict]:
    """Load QA JSON dataset (sports, art, celebrity, technology)."""
    with open(path, 'r') as f:
        data = json.load(f)
...
def load_imdb(n: int = 200) -> List[Dict]:
...
def load_aqua(path: str, n: int = 100) -> List[Dict]:
    """Load AQUA math reasoning dataset."""
    with open(path, 'r') as f:
...
def run_imdb_test(model, tokenizer, data: List[Dict], disrupted: bool = False) -> Dict:
    ...
    try:
        ...
        for item in tqdm(data, desc=desc):
            # Truncate review to fit in context
            text = item['text'][:1500]
...
def run_aqua_test(model, tokenizer, data: List[Dict], disrupted: bool = False) -> Dict:
    """Run AQUA math reasoning (contextual) test."""
    hooks = register_mean_replacement_hooks(model) if disrupted else []
    correct = 0
    total = len(data)

    try:
        desc = f"AQUA ({'disrupted' if disrupted else 'baseline'})"
        for item in tqdm(data, desc=desc):
            question = item['question']
            options = ' '.join(item['options'])
            correct_letter = item['correct']

            prompt = f"Solve this problem and choose the correct option (A, B, C, D, or E).\n\nProblem: {question}\n\nOptions: {options}\n\nAnswer:"
            ...
            match = re.search(r'\b([A-E])\)', response_upper) or re.search(r'^([A-E])\b', response_upper.strip())
            if match:
                predicted = match.group(1)
            else:
                # Find any A-E letter
                for letter in ['A', 'B', 'C', 'D', 'E']:
                    if letter in response_upper[:50]:
                        predicted = letter
                        break
                else:
                    predicted = None
...
def generate_passkey_prompt(context_length: int = 500) -> Tuple[str, str]:
...
def run_all_tests(model_name: str, model, tokenizer) -> Dict:
    """Run all parametric and contextual tests."""
    results = {'parametric': {}, 'contextual': {}}

    # ===== PARAMETRIC TESTS =====
    print(f"\n{'='*60}")
    print(f"PARAMETRIC TESTS - {model_name}")
    print('='*60)
    ...
    # ===== CONTEXTUAL TESTS =====
    print(f"\n{'='*60}")
    print(f"CONTEXTUAL TESTS - {model_name}")
    print('='*60)
    ...
    # AQUA
    aqua_data = load_aqua("Rope_with_LLM/datasets/aqua_test.json", n=50)
    print(f"\nAQUA: {len(aqua_data)} samples")
    baseline = run_aqua_test(model, tokenizer, aqua_data, disrupted=False)
    disrupted = run_aqua_test(model, tokenizer, aqua_data, disrupted=True)
    deg = (baseline['accuracy'] - disrupted['accuracy']) / baseline['accuracy'] * 100 if baseline['accuracy'] > 0 else 0
    results['contextual']['aqua'] = {'baseline': baseline['accuracy'], 'disrupted': disrupted['accuracy'], 'degradation': deg}
    print(f"AQUA: {baseline['accuracy']:.1%} → {disrupted['accuracy']:.1%} ({deg:.1f}% degradation)")
...
def main():
    ...
    all_results['RoPE'] = run_all_tests("RoPE", model, tokenizer)
    ...
    all_results['DroPE'] = run_all_tests("DroPE", model, tokenizer)
    ...
    with open(output_dir / f"jin_tests_{timestamp}.json", "w") as f:
